...
- [x] **#17 Cookie 管理器** | 工作量:中小 | 依赖:无 | ✅ 2026-06-27
  - 按域名查看/编辑/删除Cookie，导入导出Cookie Jar
  - `CookieManager.vue`(新增) + `cookie-store.ts`(新增) + `cookie-parser.ts`(新增) + `mitm-server.ts`
...
- [x] **#23 GraphQL 感知** | 工作量:小 | 依赖:无 | ✅ 2026-06-27
  - 识别GraphQL请求，按operation name分组
  - `graphql-detector.ts`(新增) + `RequestList.vue` + `FilterPanel.vue`
