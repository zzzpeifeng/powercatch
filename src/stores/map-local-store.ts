...
export const useMapLocalStore = defineStore('map-local', () => {
  // ===== State =====

  /** Map Local 规则列表 */
  const rules = ref<MapLocalRule[]>([])
  ...
  async function loadRules(): Promise<void> {
    try {
      ...
      if (settings?.mapLocalRules) {
        rules.value = settings.mapLocalRules
      }
      loaded.value = true
    } catch (error) {
      console.error('Failed to load map local rules:', error)
    }
  }
  ...
  async function saveRules(): Promise<void> {
    try {
      await ipc.settings.saveAll({ mapLocalRules: rules.value })
      // 同步到主进程 proxy 层（仅同步启用的规则）
      await ipc.mapLocal.syncRules(rules.value.filter(r => r.enabled))
    } catch (error) {
      console.error('Failed to save map local rules:', error)
    }
  }

  /** 验证规则 */
  function validateRule(rule: { name?: string; match?: { urlPattern?: string }; localPath?: string }): string[] {
    const errors: string[] = []
  ...
  /** 添加规则 */
  async function addRule(rule: Omit<MapLocalRule, 'id' | 'createdAt'>): Promise<{ success: boolean; rule?: MapLocalRule; errors: string[] }> {
    ...
    if (rules.value.length >= MAX_RULES) {
      return { success: false, errors: ['Map Local 规则最多 50 条'] }
    }
  ...
  /** 更新规则 */
  async function updateRule(ruleId: string, updates: Partial<MapLocalRule>): Promise<{ success: boolean; errors: string[] }> {
